...
def data_preparation():
    sheet = pd.read_csv(r"THF_THP_feature.csv")
...
def bayesian_opt(in_variable, target, path, delta=0.05):
    ...
    # length_scale_bounds_ = [(1e-308, 1e308)]*(numerofvariable) 
    # # noise_bounds=(1e-308, 1e308)
    # noise_bounds=(0.0001, 1e308)
    # sigma_bound = (1e-30, 1e308)    
    ...
    for t in range(T):   
        if t == 0:
            ...
            trans.fit(train_data_raw.values)           
            ...
            beta_phalf = np.sqrt(2*np.log((len(test_pred_target)*np.pi**2*(t+1)**2)/(6*delta))) 
            # beta_phalf = Beta^1/2
    ...
    return bo_reward_store, stopping_, time_to_pick_best, pulled_arm_store,\
        all_test_rmse, all_train_rmse, test_linear_correlation, all_gamma_,\
            all_threshold, bo_test_std, beta_12, kernel_values, elapsed_time1_, lenght_scale_bound

# bo_reward_store, stopping_, time_to_pick_best, pulled_arm_store, all_test_rmse, all_train_rmse, test_linear_correlation, all_gamma_, all_threshold = bayesian_opt(in_variable, target, path, delta=0.05)

#%% OFUL Calculaing many episodes

def episode_bo():
    folder_path = "Manuscript enan BO-AF 50 run"
    number_episode = 50
    ...
    for i in range(number_episode):  
        ...
        bo_reward_store, stopping_, time_to_pick_best, pulled_arm_store,\
            all_test_rmse, all_train_rmse, test_linear_correlation, all_gamma_,\
                all_threshold, bo_test_std, beta_12, kernel_values, elapsed_time1_, lenght_scale_bound\
    ...
    return stopping_time, reward_store_, time_to_get_best, pulled_arm_,\
        folder_path, test_rmse_, correlation_, path

stopping_time, reward_store_, time_to_get_best, pulled_arm_,\

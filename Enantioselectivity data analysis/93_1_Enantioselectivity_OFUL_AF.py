...
def oful_bandit(in_variable, target, path, delta=0.05):
    ...
    for t in range(T):   
        if t == 0:
            ...
            train_data = np.insert(standardized_in_variable[training_indices,:],\
                                   0, np.ones(len(train_y)), axis=1)                
            test_data  =  np.insert(np.delete(standardized_in_variable,\
                                              training_indices, 0), 0, np.ones(len(test_y)), axis=1)          
...
def episode_oful():
    folder_path = "Corrected Manuscript enan OFUL-AF 50 run"
    number_episode = 50
    ...
    for i in range(number_episode):  
        ...
        oful_reward_store, path, stopping_, lambda_, time_to_pick_best, theta_hat_store, S_store,\
            C_t_store, pulled_arm_store, all_test_rmse, all_train_rmse, test_linear_correlation,\
                all_gamma_, all_threshold, elapsed_time1_ = oful_bandit(in_variable, target, path, delta=0.05)
    ...
    return stopping_time, lambda_store, path, reward_store_, feature, time_to_get_best, theta_hat_store_, S_store_, C_t_store_, pulled_arm_, folder_path, test_rmse_, correlation_

stopping_time, lambda_store, path, reward_store_, feature, time_to_get_best, theta_hat_store_, S_store_, C_t_store_, pulled_arm_, folder_path, test_rmse_, correlation_  = episode_oful()                

...
# path = "all feature freesove5 debug1"
# 0.05/300
def bayesian_opt(in_variable, target, path, SD_, delta=0.05):
    ...
    for t in range(T):
        if t == 0:
            ...
            train_data = train_data_full_sapace[:,important_feature_index]      # train_data.shape   
            ...
            if num_important_feature == 0:               
                ...
                tuned_kernel=gpr.kernel_.get_params(deep=False)
    ...
    return bo_reward_store, stopping_, time_to_pick_best, pulled_arm_store,\
        all_test_rmse, all_train_rmse, test_linear_correlation, all_gamma_,\
            all_threshold, impo_fea_store, feature_number, bo_test_std,\
                beta_12, kernel_values, elapsed_time1_, lenght_scale_bound

#%% OFUL Calculaing many episodes

def episode_bo():
    folder_path = "Manuscript Syn2 BO-SF 50 run"
    number_episode = 50
...
stopping_time, reward_store_, time_to_get_best, theta_hat_store_,\
    pulled_arm_, folder_path, test_rmse_, correlation_, path = episode_bo()

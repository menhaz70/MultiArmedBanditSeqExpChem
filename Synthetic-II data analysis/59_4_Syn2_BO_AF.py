...
def data_synthesis():
    ...
    # y = y_wo_noise + np.random.normal(0, SD_, n_samples)
    y_ = pd.Series(y)
    X_ = pd.DataFrame(X)  
...
def bayesian_opt(in_variable, target, path, SD_, delta=0.05):
    ...
    return bo_reward_store, stopping_, time_to_pick_best, pulled_arm_store, all_test_rmse,\
        all_train_rmse, test_linear_correlation, all_gamma_, all_threshold, bo_test_std,\
...
def episode_bo():
    folder_path = "Manuscript Syn2 BO-AF 50 run"
    number_episode = 50
    ...
    return stopping_time, reward_store_, time_to_get_best, pulled_arm_, folder_path, test_rmse_, correlation_, path

stopping_time, reward_store_, time_to_get_best, pulled_arm_, folder_path, test_rmse_, correlation_, path = episode_bo()

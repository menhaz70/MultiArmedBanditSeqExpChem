...
def rdm_bandit(in_variable, target, path, delta=0.05):
    ...
    for t in range(T):   
        if t == 0:
            ...
            search_ = GridSearchCV(pipeline_, {"model__alpha": np.logspace(-2, 2, num=200)},\
            ...
            SE_theta_hat = np.sqrt(np.diag(V_inverse @ (train_data.T @ train_data) @ V_inverse)) * R  # R is the estimate of \sigma^hat
            ...
            theta_set = np.empty(shape=(0, d))        ### np.zeros(N,d) ### theta_set.shape x.shape    
            for k in range(K):  
            ...
            # t_rain_bound2 = np.empty(shape=(2, 0))        
            ...
            # for j in range(len(train_pred_target)):
            #     l_bound = train_pred_target.tolist()[j] - train_bound[1,j]
            #     u_bound = train_pred_target.tolist()[j] + train_bound[1,j]
            #     i_nterval = np.array([l_bound, u_bound]).reshape(2,1) 
            #     t_rain_bound2 = np.append(t_rain_bound2, i_nterval, axis =1)
            ...
            # plt.text(-13,-18,"%s" %column_name)
            # plt.text(-13,-15,"%s features selected" %column)                    
            # plt.text(-13,-19.5,"# common features = %s " %common_feature)                   
            ...
            oful_reward_armwise[pulled_arm].append(reward) # oful_reward_armwise[15].append(reward)         
...
def episode_rdm_bandit():
    folder_path = "Manuscript Syn2 RS 50 run"
    number_episode = 50
    ...
    for i in range(number_episode):  
        ...
        target, in_variable, coef, SD_, percen_ = data_synthesis()              
        oful_reward_store, path, stopping_, lambda_, feature_number,\
            time_to_pick_best, theta_hat_store, S_store, C_t_store,\
                pulled_arm_store, all_test_rmse, all_train_rmse,\
                    test_linear_correlation, all_gamma, all_threshold, elapsed_time1_\
    ...
    return stopping_time, lambda_store, path, reward_store_, feature, time_to_get_best,\
        theta_hat_store_, S_store_, C_t_store_, alpha_store, selected_arm_, folder_path,\
            test_rmse_, correlation_

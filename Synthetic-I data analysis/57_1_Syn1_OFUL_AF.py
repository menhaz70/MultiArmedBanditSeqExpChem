...
#%% OFUL bandit dynamic feature  (Syn1 Data)
...
def episode_oful():
    ...
    folder_path = "Corrected Manuscript Syn1 OFUL-AF 50 run"
    number_episode = 50
    ...
    for i in range(number_episode):  
        ...
        #seed_for_data_ = seed_for_data[i]    
        #seed_ = all_seed[i]

        target, in_variable, coef, SD_, percen_ = data_synthesis()              
        oful_reward_store, path, stopping_, lambda_, time_to_pick_best, theta_hat_store,\
            S_store, C_t_store, pulled_arm_store, all_test_rmse, all_train_rmse,\
                test_linear_correlation, all_gamma_, all_threshold, elapsed_time1_\
    ...
    # pd.DataFrame([elapsed_time]).to_csv("%s/elapsed_time.csv" % path)     
    
    return stopping_time, lambda_store, path, reward_store_, time_to_get_best, theta_hat_store_, S_store_, C_t_store_, alpha_store, pulled_arm_, folder_path, test_rmse_, correlation_

stopping_time, lambda_store, path, reward_store_, time_to_get_best, theta_hat_store_, S_store_, C_t_store_, alpha_store, pulled_arm_, folder_path, test_rmse_, correlation_ = episode_oful()
...
# np.random.randint(1, 100000, 20)
# np.random.randint(1, 10000000, 20)
# stats.norm.ppf(1-(0.90/2), loc=0, scale=1)
# stats.norm.ppf(1-(0.20/2), loc=0, scale=1)
# stats.norm.ppf(1-(0.50/2), loc=0, scale=1)
# stats.norm.ppf(1-(0.05/2), loc=0, scale=1)
# stats.norm.ppf(1-(0.0001/2), loc=0, scale=1)
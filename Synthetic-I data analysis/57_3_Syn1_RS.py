...
def rdm_bandit(in_variable, target, path, delta=0.05):
    ...
    for t in range(T):   
        if t == 0:
            ...
            test_data  =  np.insert(np.delete(standardized_in_variable, training_indices, 0), 0,\
    ...
    pd.DataFrame({"lambda": lambda_, "R": all_train_rmse, "S": S_store,\
                  "1st part": aa, "2nd part": bb}).to_csv("%s/parts of ellipse.csv" % path)              
    
    return stopping_, oful_reward_store, lambda_, best_alphas, time_to_pick_best, theta_hat_store, S_store, C_t_store,\
...
def episode_rdm_bandit():
    folder_path = "Corrected Manuscript Syn1-1 RS 50 run"
    number_episode = 50
    ...
    for i in range(number_episode):  
        ...
        target, in_variable, coef, SD_, percen_ = data_synthesis()              
        stopping_, oful_reward_store, lambda_, best_alphas, time_to_pick_best, theta_hat_store, S_store, C_t_store,\
            pulled_arm_store, all_test_rmse, all_train_rmse, test_linear_correlation,\
                all_gamma_, all_threshold, elapsed_time1_= rdm_bandit(in_variable, target, path, delta=0.05)
    ...
    return stopping_time, lambda_store, path, reward_store_, time_to_get_best,\
        theta_hat_store_, S_store_, C_t_store_, selected_arm_,\
            folder_path, test_rmse_, correlation_
            
topping_time, lambda_store, path, reward_store_, time_to_get_best,\
    theta_hat_store_, S_store_, C_t_store_, selected_arm_,\

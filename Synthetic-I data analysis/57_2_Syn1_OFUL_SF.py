...
def oful_bandit(in_variable, target, path, delta=0.05):
    ...
    for t in range(T):
        if t == 0:
            ...
            # Feature selection 
            pipeline = Pipeline([("scaler", StandardScaler()), ("model", Lasso(max_iter=1000, tol=0.005))])
    ...
    return oful_reward_store, path, stopping_, lambda_, feature_number, impo_fea_store,\
        best_alphas, time_to_pick_best, theta_hat_store, S_store, C_t_store, pulled_arm_store,\
            all_test_rmse, all_train_rmse, test_linear_correlation, all_gamma,\
                all_threshold, elapsed_time1_

#%% OFUL Calculaing many episodes

def episode_oful():
    folder_path = "Corrected Manuscript Syn1 OFUL-SF 50 run"
    number_episode = 50
    ...
    return stopping_time, lambda_store, path, reward_store_, feature, time_to_get_best, theta_hat_store_, S_store_, C_t_store_, alpha_store, selected_arm_, folder_path, test_rmse_, correlation_
stopping_time, lambda_store, path, reward_store_, feature, time_to_get_best, theta_hat_store_, S_store_, C_t_store_, alpha_store, selected_arm_, folder_path, test_rmse_, correlation_  = episode_oful()                

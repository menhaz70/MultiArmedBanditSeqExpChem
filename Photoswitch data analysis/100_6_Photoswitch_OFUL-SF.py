...
def data_preparation():
    sheet = pd.read_csv(r"photoswitch_data.csv")
    target = sheet["E isomer pi-pi* wavelength in nm"]
    feature = pd.read_excel(r"photoswitch_feature.xlsx")     
...
def oful_bandit(in_variable, target, path, delta=0.05):
    ...
    for t in range(T):
        if t == 0:
            ...
            best_para =  search.best_params_['model__alpha']
...
def episode_oful():
    folder_path = "Corrected Review photoswitch OFUL-SF 50 run"
    number_episode = 50
